...
@startStreamBroadcast = () ->
	...
	InstanceStatus.getCollection().find({'extraInformation.port': {$exists: true}}, {sort: {_createdAt: -1}}).observe
		added: (record) ->
			if record.extraInformation.port is process.env.PORT and (record.extraInformation.host is 'localhost' or record.extraInformation.host is process.env.NETWORK_ADDRESS)
				return
	...
	broadcast = (streamName, eventName, args, userId) ->
		fromInstance = (process.env.NETWORK_ADDRESS or 'localhost') + ':' + process.env.PORT
